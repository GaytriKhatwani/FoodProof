...
- **Data handling (provider's official documentation, checked 6 September 2026):** API inputs and outputs are not retained by default and are never used for model training under the Commercial Terms; uploaded images are not stored beyond the request and image metadata is not read; `claude-sonnet-5` is not a model that requires 30-day retention. The integration never logs evidence, prompts, extracted text or drafts; server logs carry a content-free failure reason and a ledger id only. The reporter's confirmed facts and concern text (their own words) are sent for drafting; label photographs are sent for extraction only after ownership, kind, type and size checks.
