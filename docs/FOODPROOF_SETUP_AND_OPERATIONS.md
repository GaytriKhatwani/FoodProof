...
**A shared demo code (optional).** To let any visitor walk the demo as a user without asking for a code, create one user-role invitation with a long expiry (`node --env-file=.env.local scripts/create-invitations.mjs --users 1 --no-reviewer --days 365`) and set its raw code as `DEMO_PUBLIC_USER_CODE` on Vercel. The entry page then shows it beside the invitation field with a "Use the demo code" button. Everyone who can load the page can enter as that demo user, so do this only while every record in the demo is sample or redacted material, and revoke the row (`update demo_access set revoked_at = now() where id = '<id>'`) when the demo closes. Never publish a reviewer code.
