...
Server configuration: SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY, MIXPANEL_TOKEN, MIXPANEL_API_HOST, APP_ORIGIN, RATE_LIMIT_HMAC_KEY, DEMO_MODE=true. AI_PROVIDER and provider-specific key/model are required for the AI path, which the owner commits to enabling before T4 (D33); no provider or model is selected in these documents. Use deployment secret storage and ignored local environment files. T0 creates `.env.example` with empty placeholders and exact validation rules, then README install/run/test commands verified against chosen versions.
