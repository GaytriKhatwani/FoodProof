...
**Phase two C.1 — email sign-in, when the deployment enables it.** A server component reads the deployment flag directly, never by probing an endpoint, so a deployment without it renders this screen exactly as above. When enabled, `/pilot` shows email sign-in first (“Sign in with your email”) and the invitation code second (“Have an invitation code?”), each with one explanatory line: the email path states that signing in creates or reuses an account tied to that address and never connects to any demo reports; the invitation path keeps the existing “these are test labels, not verified identities” statement. Email sign-in is two steps — send a code, then enter it — with a resend control on a short visible cooldown, and it joins the same analytics-consent step and destination as invitation entry once verified. Every sign-in failure is one generic message, matching the invitation path's own refusal to distinguish causes.
